...
def main(ctx: beet.Context):
   ...
   pot_list = [x.removeprefix('minecraft:') for x in get_registry(vanilla, 'minecraft:decorated_pot_pattern').keys()]
   ...
   for name,values in hardcoded_items.items():
      ctx.data.item_tags[f'tryashtar.shulker_preview:special_render/{name}'] = beet.ItemTag({"values":list(sorted(values.keys()))})
   ...
   for kind,items in colorable_items.items():
      if len(items) > 1:
         ctx.data.item_tags[f'tryashtar.shulker_preview:special_render/{kind}'] = beet.ItemTag({"values":list(sorted(items))})
   ...
   ctx.data.functions['tryashtar.shulker_preview:meta/initialize_data'] = beet.Function([
   ...
   def load_textures(item, model_name, model):
      ...
      for i,override in enumerate(model['overrides']):
         ...
         if not ('pulling' in predicate or 'pull' in predicate or 'brushing' in predicate or 'time' in predicate or 'angle' in predicate or 'cast' in predicate or 'tooting' in predicate or 'blocking' in predicate or 'trim_type' in predicate):
            submodel = get_model(vanilla.assets, model_cache, override['model'])
            textures = load_textures(item + '.' + str(i), with_namespace(override['model']), submodel)
            result['overrides'].append((predicate, textures['base']))
      return result
   for item in item_list:
      model_name = f'minecraft:item/{item}'
      model = get_model(vanilla.assets, model_cache, model_name)
      textures = load_textures(item, model_name, model)
   ...
   for row in range(0, 3):
      ...
      ctx.data.functions[f'tryashtar.shulker_preview:render/row_{row}/overlay/shield_base'] = beet.Function(shield_base)
      ctx.data.functions[f'tryashtar.shulker_preview:render/row_{row}/overlay/pot_patterns1'] = beet.Function([
      ...
      ctx.data.functions[f'tryashtar.shulker_preview:render/row_{row}/overlay/pot_patterns2'] = beet.Function([
      ...
      for armor,tag in {'helmet':'head_armor', 'chestplate':'chest_armor', 'leggings':'leg_armor', 'boots':'foot_armor'}.items():
         ...
         ctx.data.functions[f'tryashtar.shulker_preview:render/row_{row}/overlay/armor_trim/{armor}'] = beet.Function(armor_trim)
      ...
      ctx.data.functions[f'tryashtar.shulker_preview:render/row_{row}/overlay/armor_trim'] = beet.Function(main_trim)
      durability = [
      ...
      ctx.data.functions[f'tryashtar.shulker_preview:render/row_{row}/overlay/durability'] = beet.Function(durability)
      bundle_bar = [
      ...
      ctx.data.functions[f'tryashtar.shulker_preview:render/row_{row}/overlay/bundle_bar'] = beet.Function(bundle_bar)
      override_fn = [
      ...
      for item,predicates in override_items.items():
         ...
         ctx.data.functions[f'tryashtar.shulker_preview:render/row_{row}/special_render/{item}'] = beet.Function(specific_fn)
      ctx.data.functions[f'tryashtar.shulker_preview:render/row_{row}/special_render/overrides'] =  beet.Function(override_fn)
   grid_image = create_image(grid, 64)
   ctx.assets.textures['tryashtar.shulker_preview:block_sheet'] = beet.Texture(grid_image)
   ctx.assets.languages['tryashtar.shulker_preview:en_us'] = beet.Language(lang)
   ctx.assets.fonts['tryashtar.shulker_preview:preview'] = beet.Font({"providers":font})
   ctx.data.item_tags['tryashtar.shulker_preview:special_render'] = beet.ItemTag({"values":special_render_tag})
   ctx.data.item_tags['tryashtar.shulker_preview:special_render/overrides'] = beet.ItemTag({"values":list(sorted(override_items.keys()))})
...
def get_registry(release: beet.contrib.vanilla.Release, registry: str) -> dict:
   ...
   with open(path / "generated" / "reports" / "registries.json", encoding='utf-8') as file:
      items = json.load(file)[registry]["entries"]
   return items
...
def get_model(pack: beet.ResourcePack, cache, identifier):
   identifier = with_namespace(identifier)
   model = pack.models[identifier].data
   ...
   if parent_name is not None:
      ...
      if parent_name == 'minecraft:builtin/generated':
         ...
         parent = get_model(pack, cache, parent_name)
