...
with open("resourcepack\\assets\\minecraft\\lang\\en_us.json", "w") as file:
   num = 1
   file.write("{\"shulker_item.background\":\"\\uF820\\uF804\\uE000\\uF80C\\uF80A\\uF808\\uF801\\uF806\\uF800\",\"shulker_item.empty_slot\":\"\\uF820\\uF828\\uF822\\uF800\",")
   for item in items:
      file.write("\"shulker_item.item." +item_name(item)+ "\":\"\\uF820\\uE" +format(num,"03x").upper()+ "\\uF823\\uF806\\uF800\"")
      if item != items[-1]:
         file.write(",")
      num += 1
   file.write("}")
...
with open("datapack\\data\\shulker_item\\functions\\process_box.mcfunction", "w") as file:
   ...
   for i in range(0, 27):
      for item in items:
         file.write("execute if data block ~ ~1 ~ Items[{Slot:" +str(i)+ "b,id:\"minecraft:" +item_name(item)+ "\"}] run summon area_effect_cloud ~ ~" +str(i+4)+ " ~ {Tags:[\"shulker_item\"],CustomName:\"{\\\"translate\\\":\\\"shulker_item.item." +item_name(item)+ "\\\"}\"}\n")
