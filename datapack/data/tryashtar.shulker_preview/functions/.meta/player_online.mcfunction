...
execute if score #version shulker_preview matches 1..2681 run tellraw @a [{"text":"\nOutdated Minecraft version!\nYou need to be on version ","color":"red"},{"text":"20w46a","color":"yellow"},{"text":" or later for shulker previews to work!\n"},{"text":"Download for older versions here\n","color":"blue","underlined":true,"clickEvent":{"action":"open_url","value":"https://tryashtar.github.io/shulker-preview"}}]
execute if score #version shulker_preview matches 1..2681 run scoreboard players set #install shulker_preview -1
execute if score #version shulker_preview matches 2682.. if score #install shulker_preview matches -1 run scoreboard players set #install shulker_preview 0
