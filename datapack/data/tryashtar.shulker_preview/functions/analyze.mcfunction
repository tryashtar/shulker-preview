...
execute if score #amount shulker_preview matches ..0 run data modify block ~2 1 ~ Text1 set value '{"translate":"%1$s%418634357$s","with":["",[{"font":"tryashtar.shulker_preview:preview","text":"\\uF800"},{"selector":"@e[type=area_effect_cloud,tag=tryashtar.shulker_preview,x=0,y=0,z=0,sort=nearest]","color":"white","italic":false}]]}'
